...
def improve_text_quality(PROMPT, text_to_imporve, llm, language, jd):
    """Invoke LLM to improve the text quality."""
    query = PROMPT.format(text=text_to_imporve, language=language, jd= jd)
    response = llm.invoke(query)
    return response


def improve_work_experience(WORK_EXPERIENCE: list, llm,jd):
...
def improve_projects(PROJECTS: list, llm,jd):
...
def Evaluate_the_Resume(llm, documents,jd ):
    try:
        st.info(
...
the resume's top 3 strengths and top 3 weaknesses..."
        ...
        prompt = prompt_template.format_prompt(
            text=documents, language=st.session_state.assistant_language, jd= jd
        ).text
        ...
        try:
            ...
            list_fields = ["resume_cv_overview", "top_3_strengths", "top_corrections"]
            list_rfind = [",\n", ",\n", "\n"]
...
def Evaluate_the_Resume_keywords(llm, documents, jd , PROFESSIONAL_EXPERIENCE, SKILLS_and_CERTIF):
    try:
        st.info(
            f"**{get_current_time()}** \tEvaluate, outline and analyse \
the resume's missing keywords based on the jd..."
        )
        print(
            f"**{get_current_time()}** \tEvaluate, outline and analyse \
the resume's missing keywords based on the jd..."
        ...
        prompt_template = PromptTemplate.from_template(PROMPT_MISSING_KEY)
        prompt = prompt_template.format_prompt(
            text=documents, language=st.session_state.assistant_language,jd=jd,  PROFESSIONAL_EXPERIENCE = PROFESSIONAL_EXPERIENCE , SKILLS_and_CERTIF= SKILLS_and_CERTIF
        ).text
        ...
        try:
            RESUME_KEYWORDS = json.loads(response_content)
        except Exception as e:
            print("[ERROR] json.loads returns error:", e)
            print("\n[INFO] Parse response content...\n")

            list_fields = ["JD_Match", "MissingKeywords", "Profile_Summary"]
            list_rfind = [",\n", ",\n", "\n"]
            list_exclude_first_car = [True, True, True]
            RESUME_EVALUATION = ResponseContent_Parser(
                response_content, list_fields, list_rfind, list_exclude_first_car
...
def resume_matcher_main(llm, llm_creative, documents, jd):
    ...
    PROFESSIONAL_EXPERIENCE["Work__experience"] = improve_work_experience(
        WORK_EXPERIENCE=PROFESSIONAL_EXPERIENCE["Work__experience"], llm=llm_creative, jd =jd
    )

    # 9. Improve the quality of the project section.
    PROFESSIONAL_EXPERIENCE["CV__Projects"] = improve_projects(
        PROJECTS=PROFESSIONAL_EXPERIENCE["CV__Projects"], llm=llm_creative, jd=jd
    )

    # 10. Evaluate the Resume
    # RESUME_EVALUATION = Evaluate_the_Resume(llm_creative, documents, jd)
    
    RESUME_KEY=Evaluate_the_Resume_keywords(llm_creative, documents, jd, PROFESSIONAL_EXPERIENCE, SKILLS_and_CERTIF )
    print(RESUME_KEY)
